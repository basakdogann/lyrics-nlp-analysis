...
print("Lemmatize boyut (MB):", os.path.getsize(lemmatized_path) / (1024*1024))
print("Stemmed boyut (MB):", os.path.getsize(stemmed_path) / (1024*1024))
...
sns.lineplot(data=top_lemma, x=range(1, 51), y="frequency", marker="o", label="Lemmatize")
sns.lineplot(data=top_stem, x=range(1, 51), y="frequency", marker="o", label="Stemmed")
plt.title("Zipf Grafiği: Lemmatize vs Stemmed")
plt.xlabel("Kelime Sırası (Rank)")
plt.ylabel("Frekans")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("zipf_grafigi.png")
plt.show()
